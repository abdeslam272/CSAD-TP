...
! using the subspace iteration method with Rayleigh-Riz projection + block approach (v2) + deflation (v3)
!
! ..Input arguments
...
!-------------------------------------------------------------------------------------------------------------------
  subroutine subspace_iter_v3(n, m, a, p, percentage, maxit, eps, v, w, n_ev, acc_ev, it_ev, ierr)
    ...
    double precision, dimension(n, n)                :: atemp
    ...
    atemp = a
    do j = 1, p-1
        call dgemm('n', 'n', n, n, n ,done, atemp, n, a, n , dzero, temp, n)
        atemp = temp
    end do
    ...
    do while((eig_sum .lt. p_trace) .and. (n_ev .lt. m) .and. (k .lt. maxit))
      ...
      !! A. Compute  Y = A^p . V (version optimisé)
      call dgemm('n', 'n', n, m-n_ev, n, done, atemp, n, v(1,n_ev+1), n, dzero, y(1,n_ev+1), n)
      ...
      !!     H = V'*Y
      call dgemm('t', 'n', m-n_ev, m-n_ev, n, done, v(1,n_ev+1), n, y(1,n_ev+1), n, dzero, h(1,1), m) 
      !!   2. Spectral decomposition
      call dsyev('v', 'u', m-n_ev, h(1,1), m, w_aux, work, lwork, ierr) 
      if( ierr .ne.0 )then
