...
a1 = plot(X(:,1),X(:,2),'r+',0,0,'mo',...
    [0 5],[0 0],'c--',...
    [0 0],[0 5],'g--',...
    [M(1),5*vect(1,1)+M(1)],[M(2),5*vect(2,1)+M(2)],'k--',...
    [M(1),5*vect(1,2)+M(1)],[M(2),5*vect(2,2)+M(2)],'b--','linewidth',2);grid on,figure(1), hold on;
title('Changement de repere : repere canonique VS repere principal')
a2 = plot(M(1),M(2),'ko');
legend([a1;a2],'nuage de points','origine','premier axe canonique','deuxieme axe canonique',...
    'premier axe principal','deuxieme axe principal','individu moyen');
...
p1 = plot(Xc(:,1),Xc(:,2),'r+');grid on,figure(2), hold on;
p2 = plot(C(:,1),C(:,2),'b+');grid on,figure(2);
legend([p1;p2],'LA PROJECTION DES INDIVIDUS DE XC SUR LES DEUX PREMIERS AXES DE LA BASE CANONIQUE','LA PROJECTION DE CES INDIVIDUS SUR LES DEUX PREMIERS AXESPRINCIPAUX');
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
info_axe_1 = sigma(1,1)/sum(diag(sigma))*100;
info_axe_2 = sigma(2,2)/sum(diag(sigma))*100;
info_axe_3 = sigma(3,3)/sum(diag(sigma))*100;
